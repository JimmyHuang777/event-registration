...
// Every Car Manager's line_user_id, for pushing them an alert.
async function getCarManagerLineUserIds(): Promise<string[]> {
  const { data: rows } = await supabase.from("car_manager_admins").select("user_id");
  ...
  try {
    ...
    switch (action) {
      ...
      case "create_ride_request": {
        ...
        // Alert every Car Manager by LINE push — best-effort, never
        // blocks or fails the passenger's own request.
        try {
          const { data: eventRow } = await supabase.from("events").select("name").eq("id", event_id).maybeSingle();
          const managerLineIds = await getCarManagerLineUserIds();
          const count = requestPassengerCount({ additional_passengers: additionalPassengers });
          await pushLineNotification(
            managerLineIds,
            `🚗 新的共乘需求\n活動：${eventRow?.name || "活動"}\n乘客：${passengerName}（共 ${count} 人）\n請至共乘管理頁面配對司機。`
          );
        } catch (_err) {
